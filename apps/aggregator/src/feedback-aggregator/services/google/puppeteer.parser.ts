...
import { fromRelativeTimeToTimestamp } from '../../helpers/date.helpers';
...
export class GooglePuppeteerParser implements IParser {
  ...
  private async parseFeedbacksByDocument(page: puppeteer.Page): Promise<GoogleFeedback[]> {
    ...
    while (items.length < FEEDBACK_LIMIT) {
      let parsedItems: GoogleFeedback[] = await page.evaluate(GooglePuppeteerParser.extractItems);
      parsedItems = parsedItems.map((item) => ({
        ...item,
        date: fromRelativeTimeToTimestamp(item.date),
      }));
